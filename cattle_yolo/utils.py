                    ...
                    try:
                        ...
                        with st.expander("检测结果",expanded=False):
                            ...
                            for box in boxes:
                                ...
                                position = f"{xyxy[0]:.1f},{xyxy[1]:.1f},{xyxy[2]:.1f},{xyxy[3]:.1f}"

                                for key in labels.keys():
                                    if int(lable_index)==key:
                                        behavior = labels[key]
                                        if behavior in labels_num_dict:
                                            labels_num_dict[behavior]+=1
                                        else:
                                            labels_num_dict[behavior] = 1

                                        # 插入数据库记录
                                        source_name = source_img.name if source_img else "unknown"
...
def _display_detected_frames(conf, model, st_frame, st_text, image, source_type='video', source_name='unknown'):
    """
    逐帧推理
    ...
    :param source_type (str): 源类型 'video' 或 'webcam'
    :param source_name (str): 源名称（文件名或摄像头ID）
    :return: None
    """
    ...
    if len(res) > 0 and res[0].boxes is not None:
        ...
        for box in boxes:
            try:
                ...
                # 查找行为标签
                behavior = labels.get(lable_index, f"unknown_{lable_index}")
...
def load_model(model_path):
    ...
    try:
        # For PyTorch >=2.6, add DetectionModel to safe globals
            ...
            with st.spinner("执行中..."):
                try:
                    ...
                    while (vid_cap.isOpened()):
                        ...
                        if success:
                            # 调用方法逐帧预测
                            source_name = source_video.name if source_video else "unknown"
